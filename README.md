...
使用**充放电时间测量法**检测 PA5 上的电容按键。PA5 外接上拉电阻至 3.3V；手指触摸时并联体表电容，充电时间延长，通过与基线对比判定触摸状态。

| 参数 | 值 | 说明 |
|------|----|------|
| 引脚 | PA5 (AF1) | TIM2_CH1 复用，外部上拉至 3.3V |
| 计时精度 | 1µs | PSC=83，TIM2 时钟 84MHz |
| 放电时长 | 50µs | 每次检测前强制放电 |
| 超时保护 | 500µs | 等待上升沿的最大等待时间 |
| 触摸阈值 | 20µs | 充电时间超过基线多少判定为触摸 |

```c
tim2cap_init();          /* 初始化，无触摸状态下自动校准基线（耗时约 5ms） */

uint8_t cap = tim2cap_scan();   /* 0 = 未触摸，1 = 触摸按下 */
```

建议以 5~10ms 为周期轮询调用 `tim2cap_scan()`。检测按下/松开边沿：
...
while (1) {
    ...
    if (cap && !prev) {
        led_on(LED1);
        press_tick = delay_get_tick();         /* 记录按下时刻 */
    } else if (!cap && prev) {
        led_off(LED1);
        uint32_t dur = delay_get_tick() - press_tick;   /* 按压时长（ms） */
    }
    prev = cap;
}
```
